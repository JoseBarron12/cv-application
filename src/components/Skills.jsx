...
export function SkillsSection({userSkill, setUserSkill}) {
    ...
    return (
        <section className="skill-section">
            ...
            {showSection && showAddBtn && userSkill.length != 0 && userSkill.map(input => {
                return (
            ...
            {showSection && showSkillForm && <SkillSectionForm userSkill={userSkill} setUserSkill={setUserSkill} setShowAddbtn={setShowAddBtn}
             setShowSkillForm={setShowSkillForm} selectedSkill={selectedSkill} setSelectedSkill={setSelectedSkill}/>}
...
function SkillSectionForm({userSkill, setUserSkill, setShowAddbtn, setShowSkillForm, selectedSkill, setSelectedSkill}) {
    ...
    return (
        <form className="skill-info">
            ...
            <div className="additional-achievement-section">
                <button 
                    type="button"
                    onClick={onBtnClick(showSkills)}>
                        Add Skill
                        {!showSkills && <Icon path={mdiPlus} className="link-icon plus-icon" />}
                        {showSkills && <Icon path={mdiMinus} className="link-icon plus-icon" />}
                    </button>
                    {showSkills && <SkillSectionFormAdditional skillsArr={skillsArr} setSkillsArr={setSkillsArr} currentId={currentId} setCurrentId={setCurrentId} userSkill={userSkill} setUserSkill={setUserSkill}/>}
            </div>

            <SkillsSectionFormBtns setShowSkillForm={setShowSkillForm} setShowAddBtn={setShowAddbtn} currentId={currentId} setCurrentId={setCurrentId} selectedSkill={selectedSkill} setSelectedSkill={setSelectedSkill}
             userSkill={userSkill} setUserSkill={setUserSkill} categoryName={categoryName} skillsArr={skillsArr}/>
...
const processGeneralInput = (value, type, setUserSkill, userSkill, currentId, setCurrentId,
    categoryName, skillsArr) => {    
        if(currentId != null)
            {
                const arr = [...userSkill];
                const indexOfSelected = arr.findIndex((element) => {
...
function SkillInput({type, id, name, initialValue, setUserSkill, userSkill, callBack, currentId, setCurrentId,
    categoryName, skillsArr}) {
    ...
    return (
        <div>
            <input type={type} id={id + "-name"} value={currentValue} name={name + "-name"} 
            onChange={(event) => {
                    event.preventDefault();
                    
                    callBack(event.target.value)

                    processGeneralInput(event.target.value, name, setUserSkill, userSkill, currentId, setCurrentId, categoryName, skillsArr);
            ...
            {displayX && <ResetBtn setCurrentValue={setCurrentValue} setDisplayX={setDisplayX} currentId={currentId} setCurrentId={setCurrentId}
             name={name} callBack={callBack} setUserSkill={setUserSkill} userSkill={userSkill} categoryName={categoryName} skillsArr={skillsArr}/> }
        </div>
...
function ResetBtn({setCurrentValue, setDisplayX, currentId, setCurrentId, name, callBack,
    setUserSkill, userSkill, categoryName, skillsArr}) {
    return (
        <button type="button"  onClick={(event) => {
            event.preventDefault()
            
            callBack(event.target.value);
            processGeneralInput("", name, setUserSkill, userSkill, currentId, setCurrentId, categoryName, skillsArr )
...
function SkillSectionFormAdditional({skillsArr, setSkillsArr, currentId, setCurrentId, userSkill, setUserSkill}) {
    ...
    return (
            <fieldset>
                ...
                <div className="input-field"
                    ...
                    <div><input type="text" name="addit-info" id="addit-info" value={currentSkill}
                            onChange={(event) => {
                                setCurrentSkill(event.target.value);
                            }}/>
                    </div>
                </div>
                { skillsArr.length != 0 &&  skillsArr.map((skill) => {
                    return (
                    <div className="achievement-container" key={skill.id}>
                        <p>{skill.skill}</p>
                        <button className="achievement-btn"><Icon path={mdiEyeOutline} onClick={(e) => {e.preventDefault()}} className="link-icon"/></button>
...
function SkillsSectionFormBtns({setShowSkillForm, setShowAddBtn, currentId, setCurrentId, selectedSkill, setSelectedSkill,
    userSkill, setUserSkill, categoryName, skillsArr}) {
    return (
        <div className="form-btns">
            <button className="delete-btn" type="button" onClick={(e) => {
                ...
                if(selectedSkill != null)
                    {
                        const arr = [...userSkill];
                        setUserSkill(arr.filter((element) => {
