...
function GeneralSection({ generalInput, setGeneralInput }) {
  ...
  return (
    <section className="general-section">
      ...
      {showSection && (
        <GeneralSectionForm
          userInput={generalInput}
          callBack={setGeneralInput}
        />
      )}
      {showSection && (
        <div className="additional-link-section">
          <button type="button" onClick={onBtnClick(showLink)}>
            Additional Links
            {!showLink && (
  ...
  return (
    <form className="general-info">
      <div
        ...
        onClick={() => {
          addActive("general-name");
        }}
        onMouseEnter={() => {
          addActive("general-name");
        }}
        onMouseLeave={() => {
          removeActive("general-name");
        }}
      >
        ...
        <GeneralInput
          ...
          callBack={callBack}
          userInput={userInput}
          initialValue={userInput.name}
        />
      </div>
      <div
        className="input-field"
        onClick={() => {
          addActive("general-email");
        }}
        onMouseEnter={() => {
          addActive("general-email");
        }}
        onMouseLeave={() => {
          removeActive("general-email");
        }}
      >
      ...
      <div
        className="input-field"
        onClick={() => {
          addActive("general-phone");
        }}
        onMouseEnter={() => {
          addActive("general-phone");
        }}
        onMouseLeave={() => {
          removeActive("general-phone");
        }}
      >
        ...
        {supportedLinks.map((link, index) => {
          if (link == "LinkedIn") {
            return (
              <div
                className="input-field"
                key={userInput.id + index}
                onClick={() => {
                  addActive("general-link-" + link.toLocaleLowerCase());
                }}
                onMouseEnter={() => {
                  addActive("general-link-" + link.toLocaleLowerCase());
                }}
                onMouseLeave={() => {
                  removeActive("general-link-" + link.toLocaleLowerCase());
                }}
              >
                <label htmlFor={"general-link-" + link.toLocaleLowerCase()}>
                  {link}
                </label>
                <GeneralInput
                  type={link.toLocaleLowerCase()}
                  name={"general-link-" + link.toLocaleLowerCase()}
                  id={"general-link-" + link.toLocaleLowerCase()}
                  callBack={callBack}
                  userInput={userInput}
                  initialValue={userInput.linkedIn}
                />
              </div>
            );
          } else if (link == "GitHub") {
